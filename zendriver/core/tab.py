...
class Tab(Connection):
    ...
    async def cf_find_interactive_challenge(
        ...
        for host_node in shadow_host_nodes:
            ...
            for shadow_root_node in host_node.shadow_roots:
                ...
                shadow_root_element = Element(shadow_root_node, self, shadow_root_node)
    ...
    async def cf_wait_for_interactive_challenge(
        ...
        while loop.time() - start_time < timeout:
            ...
            (
                ...
                shadow_root_element,
                challenge_iframe,
            ) = await self.cf_find_interactive_challenge()
    ...
    async def cf_is_interactive_challenge_present(self, timeout: float = 5) -> bool:
        """
        ...
        _, _, challenge_iframe = await self.cf_wait_for_interactive_challenge(timeout)
    ...
    async def verify_cf(
        ...
        (
            ...
            shadow_root_element,
            challenge_iframe,
        ) = await self.cf_wait_for_interactive_challenge(timeout)
        ...
        box_model_result = await self.send(
        ...
        for x_corner, y_corner in corners:
            await self.flash_point(x=x_corner, y=y_corner, duration=10)
        ...
        async def check_input(
            ...
            if (input_el.attrs.get("value") or not fresh_input) and ckbx_clckd:
                ...
                logger.debug(
                    "Input element check successful (disappeared or has value)."
                )
                return False
            return True
        ...
        while await check_input(
            ...
            try:
                await self.mouse_click(click_x, click_y)
